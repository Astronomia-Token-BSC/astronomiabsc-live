import { getSplitAccountName, getAllowance, setupEstBuyToken, setupEstSellToken, setupWeb3, setupNetwork, setupContract, setupTokenContract, setupBalance, setupTokenBalance, addEthereumAccounts, web3LoadingError } from "./actions";
...
export const loadBlockchain = async(dispatch) =>{
    try {
        ...
        if(Web3.givenProvider){
            ...
            dispatch(getSplitAccountName(String(accounts[0]).slice(0,5) + "..." + String(accounts[0]).slice(38,String(accounts[0]).length)))
...
export const getTokenBalane = async(contract, accounts, dispatch)=>{
    const receipt = await contract.methods.balanceOf(accounts[0]).call({from:accounts[0]})
    console.log("TOken Balance = ",receipt)
    dispatch(setupTokenBalance(receipt))
}

export const CheckAllowance = async(contract, accounts, dispatch)=>{
    const receipt = await contract.methods.allowance(accounts[0],ROUTER_ADDRESS).call({from:accounts[0]})
...
export const Approving = async(contract, accounts, transaction ,dispatch)=>{
    const receipt = await contract.methods.approve(ROUTER_ADDRESS, transaction).send({from:accounts[0]})
    const updateAllowance = await contract.methods.allowance(accounts[0],ROUTER_ADDRESS).call({from:accounts[0]})
...
export const getEstimatedAmount = async(contract, accounts, transaction, dispatch)=>{
    if(transaction.value != null && transaction.value != undefined && transaction.value != NaN){
        const receipt = await contract.methods.getAmountsOut(String(web3.utils.toWei(String(transaction.value),"ether")),transaction.path).call({from:accounts[0]})
        dispatch(setupEstBuyToken(receipt[1]))
        console.log(receipt[1])
    }
}

export const getEstimatedSellAmount = async(contract, accounts, transaction, dispatch)=>{
    if(transaction.value != null && transaction.value != undefined && transaction.value != NaN){
        const receipt = await contract.methods.getAmountsOut(String(web3.utils.toWei(String(transaction.value),"gwei")),transaction.path).call({from:accounts[0]})
...
export const SwapTokenToBNB = async(contract, accounts, transaction, dispatch)=>{
    console.log("before transaction");
    const receipt = await contract.methods.swapExactTokensForETHSupportingFeeOnTransferTokens(String(web3.utils.toWei(String(transaction.amountIn),"gwei")),transaction.amountOutMin,transaction.path,transaction.to,transaction.deadline)
    .send({
        from: accounts[0],
    })
    console.log("after  transaction ", receipt);
